...
const ManajemenAnggotaEditPage: Component = () => {
      ...
      <div class="flex-1 p-8 pt-20 md:pt-9 md:ml-20">
        ...
        <div class="bg-gradient-to-br from-[#f6fff9]/80 to-[#e7f6ec]/90 rounded-2xl border border-[#e1eebc] shadow-lg backdrop-blur-md p-6 flex flex-col md:flex-row gap-6 items-center max-w-2xl mx-auto mb-4 transition-all duration-200 hover:shadow-2xl hover:ring-2 hover:ring-black/10">
          ...
          <div class="flex-1 flex flex-col gap-2">
            <label class="text-[#388e5c] text-sm font-semibold">Nama lengkap
              <input type="text" class="block w-full mt-1 px-3 py-2 rounded bg-white text-[#388e5c] focus:outline-none focus:ring-2 focus:ring-[#388e5c]" value={user().nama} onInput={e => setUser(u => ({ ...u, nama: e.currentTarget.value }))} />
            </label>
            <label class="text-[#388e5c] text-sm font-semibold">Kelas
              <input type="text" class="block w-full mt-1 px-3 py-2 rounded bg-white text-[#388e5c] focus:outline-none focus:ring-2 focus:ring-[#388e5c]" value={user().kelas} onInput={e => setUser(u => ({ ...u, kelas: e.currentTarget.value }))} />
            </label>
            <label class="text-[#388e5c] text-sm font-semibold">Email
              <input type="email" class="block w-full mt-1 px-3 py-2 rounded bg-white text-[#388e5c] focus:outline-none focus:ring-2 focus:ring-[#388e5c]" value={user().email} onInput={e => setUser(u => ({ ...u, email: e.currentTarget.value }))} />
            </label>
            <label class="text-[#388e5c] text-sm font-semibold">Password
              <input type="password" class="block w-full mt-1 px-3 py-2 rounded bg-white text-[#388e5c] focus:outline-none focus:ring-2 focus:ring-[#388e5c]" value={user().password} onInput={e => setUser(u => ({ ...u, password: e.currentTarget.value }))} />
            </label>
            <div class="text-xs text-[#388e5c] mt-2">Dibuat pada: 12/03/2024</div>
          </div>
        </div>
        <div class="flex flex-col md:flex-row gap-4 max-w-2xl mx-auto mb-4">
          <div class="flex-1 bg-gradient-to-br from-[#f6fff9]/80 to-[#e7f6ec]/90 rounded-2xl border border-[#e1eebc] shadow-lg backdrop-blur-md p-4 flex items-center justify-between text-[#388e5c] text-base transition-all duration-200 hover:shadow-2xl hover:ring-2 hover:ring-black/10">
            <span>Status akun: <span class={`font-semibold ml-2 ${user().aktif ? 'text-green-700' : 'text-gray-400'}`}>{user().aktif ? 'Aktif' : 'Nonaktif'}</span></span>
            <button
              type="button"
              class={`relative inline-flex h-6 w-12 items-center rounded-full transition-colors duration-200 focus:outline-none ${user().aktif ? 'bg-green-400' : 'bg-gray-300'}`}
              onClick={() => setUser(u => ({ ...u, aktif: !u.aktif }))}
            >
              <span class="sr-only">Toggle Aktif</span>
              <span
                class={`inline-block h-5 w-5 rounded-full bg-white shadow transform transition-transform duration-200 ${user().aktif ? 'translate-x-6' : 'translate-x-1'}`}
              ></span>
            </button>
          </div>
          <div class="flex-1 bg-gradient-to-br from-[#f6fff9]/80 to-[#e7f6ec]/90 rounded-2xl border border-[#e1eebc] shadow-lg backdrop-blur-md p-4 flex items-center justify-between text-[#388e5c] text-base transition-all duration-200 hover:shadow-2xl hover:ring-2 hover:ring-black/10">
            <span>Role akun admin: <span class={`font-semibold ml-2 ${user().admin ? 'text-yellow-700' : 'text-gray-400'}`}>{user().admin ? 'Aktif' : 'Nonaktif'}</span></span>
            <button
              type="button"
              class={`relative inline-flex h-6 w-12 items-center rounded-full transition-colors duration-200 focus:outline-none ${user().admin ? 'bg-yellow-400' : 'bg-gray-300'}`}
              onClick={() => setUser(u => ({ ...u, admin: !u.admin }))}
            >
              <span class="sr-only">Toggle Admin</span>
              <span
                class={`inline-block h-5 w-5 rounded-full bg-white shadow transform transition-transform duration-200 ${user().admin ? 'translate-x-6' : 'translate-x-1'}`}
              ></span>
            </button>
          </div>
        </div>
        <div class="flex flex-col md:flex-row gap-4 max-w-2xl mx-auto">
          <button class="flex-1 bg-gradient-to-r from-[#97cb8a] to-[#6db37e] hover:from-[#388e5c] hover:to-[#8fcb8c] text-white font-bold text-xl py-3 rounded-full shadow transition-all duration-200" onClick={() => setShowModal('konfirmasi')}>Simpan</button>
          <button class="flex-1 bg-gradient-to-r from-[#97cb8a] to-[#6db37e] hover:from-[#388e5c] hover:to-[#8fcb8c] text-white font-bold text-xl py-3 rounded-full shadow transition-all duration-200" onClick={() => navigate('/kelola-anggota/detail')}>Kembali</button>
        </div>
        ...
        {showModal() === 'konfirmasi' && (
          <div class="fixed inset-0 flex items-center justify-center z-50 bg-black/40">
            <div class="bg-white rounded-xl shadow-lg p-8 max-w-xs w-full flex flex-col items-center">
              <div class="font-bold text-lg mb-4 text-center">Anda yakin ingin menyimpan perubahan data anggota?</div>
        ...
        {showModal() === 'berhasil' && (
          <div class="fixed inset-0 flex items-center justify-center z-50 bg-black/40">
            <div class="bg-white rounded-xl shadow-lg p-8 max-w-xs w-full flex flex-col items-center">
              <div class="font-bold text-lg mb-4 text-center">Data anggota berhasil diedit!</div>
              <button class="px-8 py-2 rounded bg-[#8fcb8c] text-white font-semibold mt-2 hover:bg-[#388e5c] transition" onClick={() => { setShowModal(''); navigate('/kelola-anggota/detail'); }}>OK</button>
            </div>
          </div>
        )}
...
export default ManajemenAnggotaEditPage;
